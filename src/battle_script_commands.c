...
static void Cmd_attackcanceler(void)
{
    ...
    if ((gProtectStructs[gBattlerTarget].bounceMove || gBattleMons[gBattlerTarget].ability == ABILITY_MAGIC_BOUNCE) && gBattleMoves[gCurrentMove].flags & FLAG_MAGIC_COAT_AFFECTED)
    {
...
static void Cmd_damagecalc(void)
{
    u32 sideStatus = gSideStatuses[GET_BATTLER_SIDE(gBattlerTarget)];
...
static void Cmd_typecalc(void)
{
    ...
    else
    {
		...
		{
			gBattleMoveDamage = gBattleMoveDamage * 75;
    ...
    if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT))
    {
        ...
        else
        {
            ...
            else // hp goes down
            {
                ...
                if (gBattleMoves[gCurrentMove].category == 0 && !(gHitMarker & HITMARKER_PASSIVE_DAMAGE) && gCurrentMove != MOVE_PAIN_SPLIT)
                {
                ...
                else if (gBattleMoves[gCurrentMove].category == 1 && !(gHitMarker & HITMARKER_PASSIVE_DAMAGE))
                {
...
static void Cmd_switchineffects(void)
{
		...
		&& gBattleMons[gActiveBattler].ability != ABILITY_LEVITATE)
	{
		...
		if (gBattlescriptCurrInstr[1] == BS_TARGET)
			gBattlescriptCurrInstr = BattleScript_StickyWebOnTarget;
		else if (gBattlescriptCurrInstr[1] == BS_ATTACKER)
			gBattlescriptCurrInstr = BattleScript_StickyWebOnAttacker;
		else
			gBattlescriptCurrInstr = BattleScript_StickyWebOnFaintedBattler;
	}
		...
		&& !(gBattleMons[gActiveBattler].status1 & STATUS1_ANY))
	{
		...
		else 
		{
			...
			gBattlescriptCurrInstr = BattleScript_ToxicSpikesOnAttacker;
		}
	}
	else if (!(gSideStatuses[GetBattlerSide(gActiveBattler)] & SIDE_STATUS_STEALTH_ROCK_DAMAGED)
		&& (gSideStatuses[GetBattlerSide(gActiveBattler)] & SIDE_STATUS_STEALTH_ROCK))
	{
		u8 stealthRockDmgFraction = 8;
		...
		switch (gBattleMons[gActiveBattler].type1)
		{
			case TYPE_FIRE:
			case TYPE_ICE:
			case TYPE_FLYING:
			case TYPE_BUG:
				stealthRockDmgFraction /= 2;
				break;
			case TYPE_FIGHTING:
			case TYPE_GROUND:
			case TYPE_STEEL:
		...
		{
			switch (gBattleMons[gActiveBattler].type2)
			{
				case TYPE_FIRE:
				case TYPE_ICE:
				case TYPE_FLYING:
		...
		gBattleMoveDamage = gBattleMons[gActiveBattler].maxHP / (stealthRockDmgFraction);
		if (gBattleMoveDamage == 0)
			gBattleMoveDamage = 1;
